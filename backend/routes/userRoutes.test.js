...
// Helper function for creating mock database responses
...
describe('User Routes', () => {
    ...
    describe('POST /register', () => {
        ...
        it('should return error if username, email, or password is missing', async () => {
            ...
            expect(res.body.message).toBe('Username, email, password, and subscription type are required');
        });
    ...
    describe('PUT /profile', () => {
        it('should update user profile successfully', async () => {
           const user = { user_id: '123', username: 'newdoe', email: 'newdoe@example.com', role: 'reviewer' }
    ...
    describe('GET /admin/users', () => {
        it('should return a list of users', async () => {
            ...
            pool.query = mockDbResponse([users]);
            const res = await request(app)
                .get('/admin/users')
                .set('Authorization', 'Bearer validToken');
            expect(res.status).toBe(200);
    ...
    describe('PUT /admin/users/:user_id', () => {
        it('should update the user by user_id successfully', async () => {
      ...
      it('should return error when email is not valid', async () => {
          const res = await request(app)
                .put('/admin/users/123')
                  .set('Authorization', 'Bearer validToken')
                 .send({ username: 'newdoe', email: 'invalid-email', role: 'admin' });
          expect(res.status).toBe(400);
          expect(res.body.message).toBe('Invalid email format.');
      });
    ...
    describe('DELETE /admin/users/:user_id', () => {
        it('should delete a specific user by user id', async () => {
              pool.query = mockDbResponse([{ affectedRows: 1 }]);
            const res = await request(app)
      ...
      describe('PUT /admin/users/:user_id/roles', () => {
        it('should update the role of user by user_id successfully', async () => {
